  ...
  try {
    ...
    if (!token_verification) {
      return res
        ...
        .json({ error: "Verification failed! Token has been expired" });
    }
...
const resendVerificationEmail = asyncHandler(async (req, res) => {
  try {
    ...
    if (existingToken) {
      await EmailVerToken.deleteOne({ _id: existingToken._id });
    }
...
const login = asyncHandler(async (req, res) => {
  try {
    ...
    if (user.status === "blocked") {
      return res
        .status(StatusCodes.UNAUTHORIZED)
        .json({ error: "Your account is blocked by administrator" });
    }
    if (!user.isVerified) {
      // Check if a valid verification token exists for this user
      const tokenExists = await EmailVerToken.findOne({ userId: user._id });
...
const refreshToken = asyncHandler(async (req, res) => {
  try {
    ...
    if (error.message === "jwt expired") {
      return res
        .status(StatusCodes.UNAUTHORIZED)
        .json({ error: "Token has been expired, login again." });
    }
    return res
      .status(error.statusCode || StatusCodes.INTERNAL_SERVER_ERROR)
      .json({ error: error.message });
...
const logout = asyncHandler(async (req, res) => {
  try {
    ...
    res.cookie("session_id", null, { expires: new Date(0), httpOnly: true });
...
const resetPassword = asyncHandler(async (req, res) => {
  try {
    ...
    const userToken = await PasswordVerToken.findOne({
      passwordToken: token,
    });
