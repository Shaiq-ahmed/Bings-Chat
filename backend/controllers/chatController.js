...
const CreateGroupChat = async (req, res, next) => {
        ...
        try {
            const uploadResult = await new Promise((resolve, reject) => {
              const stream = cloudinary.uploader.upload_stream(
                { folder: "group_chats", resource_type: "image" },
                (error, result) => {
    ...
    try {
        ...
        const data = {
            ...
            groupAdmin: user,
            users: [...users, user],
            img: groupImageUrl ? groupImageUrl : ''
        }
...
const updateGroupImage = async (req, res, next) => {
    ...
    try {
        ...
        const updateGroup = await Chat.findOneAndUpdate({ _id: chatId }, { img: groupImageUrl }, { new: true })

        const findChat = await Chat.findOne({ _id: updateGroup._id })
            .populate("users", "name email phone avatar bio lastSeen")
            .populate("groupAdmin", "name email phone avatar bio lastSeen")
